...
def url_list(consul, key):
    return list(json.loads(consul.kv.get(key)[1]['Value'].decode('ascii')).values())
...
class S(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        url = random.choice(url_list(consul, "LOGGING"))
...
if __name__ == "__main__":
    ...
    if len(argv) != 3:
        raise ValueError("Second arg: host, third: port")
    host, port = argv[1], argv[2]

    url = f"http://{host}:{port}"
    consul = Consul()
    print(f"Updating urls for facade in consul: {url})")
    consul.kv.put("FACADE", json.dumps(url))
